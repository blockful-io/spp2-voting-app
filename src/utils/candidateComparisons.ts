import { HeadToHeadMatch, CandidateHeadToHeadResults, Allocation } from "./types";
...
export function filterHeadToHeadMatches(
  ...
  const matches = headToHeadMatches.filter(match => match.choice1.name === candidateName || match.choice2.name === candidateName);

  return { 
    matches,
    wins: matches.filter(match => match.winner === candidateName).length,
    losses: matches.filter(match => match.winner !== candidateName && match.winner !== "tie").length,
